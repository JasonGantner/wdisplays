...
G_DEFINE_TYPE_WITH_CODE(WdHeadForm, wd_head_form, GTK_TYPE_GRID, G_ADD_PRIVATE(WdHeadForm))
...
static void head_form_update_sensitivity(WdHeadForm *form) {
  ...
  for (GList *child = children; child != NULL; child = child->next) {
    ...
    if (widget != priv->enabled) gtk_widget_set_sensitive(widget, enabled_toggled);
  }
}
...
static void mode_spin_changed(GtkSpinButton *spin_button, gpointer data) {
  ...
  if (strcmp(gtk_widget_get_name(GTK_WIDGET(spin_button)), "width") == 0) mode.width = gtk_spin_button_get_value(spin_button);
...
static void wd_head_form_class_init(WdHeadFormClass *class) {
  ...
  signals[CHANGED] = g_signal_new("changed", G_OBJECT_CLASS_TYPE(class), G_SIGNAL_RUN_LAST,
                                  G_STRUCT_OFFSET(WdHeadFormClass, changed), NULL, NULL, NULL, G_TYPE_NONE, 1, G_TYPE_INT);

  gtk_widget_class_set_template_from_resource(widget_class, WDISPLAYS_RESOURCE_PREFIX "/head.ui");
...
static int32_t get_rotate_value(enum wl_output_transform transform) {
  if (transform == WL_OUTPUT_TRANSFORM_90 || transform == WL_OUTPUT_TRANSFORM_FLIPPED_90) return 90;
  else if (transform == WL_OUTPUT_TRANSFORM_180 || transform == WL_OUTPUT_TRANSFORM_FLIPPED_180) return 180;
  else if (transform == WL_OUTPUT_TRANSFORM_270 || transform == WL_OUTPUT_TRANSFORM_FLIPPED_270) return 270;
  return 0;
}
...
static void wd_head_form_init(WdHeadForm *form) {
  ...
  static const GVariantType *const mode_types[] = {G_VARIANT_TYPE_INT32, G_VARIANT_TYPE_INT32, G_VARIANT_TYPE_INT32};
  GSimpleAction *action = g_simple_action_new_stateful("mode", g_variant_type_new_tuple(mode_types, G_N_ELEMENTS(mode_types)),
  ...
  action = g_simple_action_new_stateful(ROTATE_PREFIX, G_VARIANT_TYPE_INT32, g_variant_new_int32(0));
  g_action_map_add_action(G_ACTION_MAP(head_actions), G_ACTION(action));
...
void wd_head_form_update(WdHeadForm *form, const struct wd_head *head, enum wd_head_fields fields) {
  ...
  if (fields & WD_FIELD_DESCRIPTION) gtk_label_set_text(GTK_LABEL(priv->description), head->description);
  ...
  if (fields & WD_FIELD_ENABLED) gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(priv->enabled), head->enabled);
  if (fields & WD_FIELD_SCALE) gtk_spin_button_set_value(GTK_SPIN_BUTTON(priv->scale), head->scale);
  ...
  if (fields & WD_FIELD_TRANSFORM) {
    ...
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(priv->flipped), head->transform == WL_OUTPUT_TRANSFORM_FLIPPED
  ...
  // Sync state
  if (fields & WD_FIELD_ENABLED) head_form_update_sensitivity(form);
  g_signal_emit(form, signals[CHANGED], 0);
}

GtkWidget *wd_head_form_new(void) { return gtk_widget_new(WD_TYPE_HEAD_FORM, NULL); }
...
gboolean wd_head_form_has_changes(WdHeadForm *form, const struct wd_head *head) {
  ...
  WdHeadFormPrivate *priv = wd_head_form_get_instance_private(form);
  if (head->enabled != gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(priv->enabled))) return TRUE;
  ...
  if (old_scale != new_scale) return TRUE;
  if (head->x != gtk_spin_button_get_value(GTK_SPIN_BUTTON(priv->pos_x))) return TRUE;
  if (head->y != gtk_spin_button_get_value(GTK_SPIN_BUTTON(priv->pos_y))) return TRUE;
  int w = head->mode != NULL ? head->mode->width : head->custom_mode.width;
  if (w != gtk_spin_button_get_value(GTK_SPIN_BUTTON(priv->width))) return TRUE;
  int h = head->mode != NULL ? head->mode->height : head->custom_mode.height;
  if (h != gtk_spin_button_get_value(GTK_SPIN_BUTTON(priv->height))) return TRUE;
  int r = head->mode != NULL ? head->mode->refresh : head->custom_mode.refresh;
  if (r / 1000. != gtk_spin_button_get_value(GTK_SPIN_BUTTON(priv->refresh))) return TRUE;
  if (g_variant_get_int32(g_action_get_state(priv->rotate_action)) != get_rotate_value(head->transform)) return TRUE;
  bool flipped = head->transform == WL_OUTPUT_TRANSFORM_FLIPPED || head->transform == WL_OUTPUT_TRANSFORM_FLIPPED_90
                 || head->transform == WL_OUTPUT_TRANSFORM_FLIPPED_180 || head->transform == WL_OUTPUT_TRANSFORM_FLIPPED_270;
  if (flipped != gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(priv->flipped))) return TRUE;
  return FALSE;
}
