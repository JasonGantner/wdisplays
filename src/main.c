...
static bool has_changes(const struct wd_state *state) {
  ...
  for (GList *form_iter = forms; form_iter != NULL; form_iter = form_iter->next)
    if (wd_head_form_has_changes(WD_HEAD_FORM(form_iter->data), g_object_get_data(G_OBJECT(form_iter->data), "head")))
      return true;
  return false;
}
...
static void apply_state(struct wd_state *state) {
  ...
  if (!state->apply_pending) {
    ...
    state->apply_idle    = g_idle_add_full(G_PRIORITY_DEFAULT, send_apply, state, NULL);
  }
}
...
static void update_tick_callback(struct wd_state *state) {
  ...
  if (!any_animate && !state->capture) {
    ...
    state->canvas_tick = gtk_widget_add_tick_callback(state->canvas, redraw_canvas, state, NULL);
  }
...
static void update_cursor(struct wd_state *state) {
  ...
  if (any_hovered) gdk_window_set_cursor(window, state->grab_cursor);
  else if (state->clicked != NULL) gdk_window_set_cursor(window, state->grabbing_cursor);
  else if (state->panning) gdk_window_set_cursor(window, state->move_cursor);
...
static inline void flip_anim(uint64_t *timer, uint64_t tick) {
  ...
  if (tick < animate_end) *timer = tick - (animate_end - tick);
  else *timer = tick;
}

static void update_hovered(struct wd_state *state, gdouble mouse_x, gdouble mouse_y) {
  if (!gtk_widget_get_realized(state->canvas)) return;
  ...
  wl_list_for_each(render, &state->render.heads, link) {
    ...
    if (any_hovered) continue;
    if (state->clicked == render) {
      render->hovered = TRUE;
      any_hovered     = TRUE;
    } else if (state->clicked == NULL) {
      if (mouse_x >= render->x1 && mouse_x < render->x2 && mouse_y >= render->y1 && mouse_y < render->y2) {
        render->hovered = TRUE;
        any_hovered     = TRUE;
      }
    }
    if (init_hovered != render->hovered) flip_anim(&render->hover_begin, tick);
  }
  update_cursor(state);
  update_tick_callback(state);
}

static inline void color_to_float_array(GtkStyleContext *ctx, const char *color_name, float out[4]) {
  GdkRGBA color;
...
static void queue_canvas_draw(struct wd_state *state) {
  ...
  color_to_float_array(style_ctx, "theme_selected_bg_color", state->render.selection_color);
  ...
  for (GList *form_iter = forms; form_iter != NULL; form_iter = form_iter->next) {
    ...
    if (wd_head_form_get_enabled(form)) {
      ...
      if (render->queued.rotation & 1) SWAP(int, w, h);
...
static void show_apply(struct wd_state *state) {
  ...
  if (has_changes(state)) {
    if (state->autoapply) apply_state(state);
    else page = "apply";
  }
  gtk_stack_set_visible_child_name(GTK_STACK(state->header_stack), page);
}

static void update_ui(WdHeadForm *form, enum wd_head_fields fields, gpointer data) {
  struct wd_state *state = data;
  show_apply(state);
...
void wd_ui_reset_head(const struct wd_head *head, enum wd_head_fields fields) {
  if (head->state->stack == NULL) return;
  ...
  for (GList *form_iter = forms; form_iter != NULL; form_iter = form_iter->next) {
    ...
    if (head == other) {
      if (fields & WD_FIELD_NAME) gtk_container_child_set(GTK_CONTAINER(head->state->stack), form, "title", head->name, NULL);
...
void wd_ui_apply_done(struct wd_state *state, struct wl_list *outputs) {
  ...
  if (!state->autoapply) show_apply(state);
  state->reset_idle = g_idle_add_full(G_PRIORITY_DEFAULT, apply_done_reset, state, NULL);
}
...
static void cleanup(GtkWidget *window, gpointer data) {
  struct wd_state *state = data;
  if (state->reset_idle != -1) g_source_remove(state->reset_idle);
  if (state->apply_idle != -1) g_source_remove(state->apply_idle);
...
static inline bool size_changed(const struct wd_render_head_data *render) {
  return render->x2 - render->x1 != render->tex_width || render->y2 - render->y1 != render->tex_height;
}
...
static cairo_surface_t *draw_head(PangoContext *pango, struct wd_render_data *info, const char *name, unsigned width,
                                  unsigned height) {
...
static void canvas_render(GtkGLArea *area, GdkGLContext *context, gpointer data) {
  ...
  wl_list_for_each(head, &state->heads, link) {
    ...
    if (output != NULL && !wl_list_empty(&output->frames)) frame = wl_container_of(output->frames.prev, frame, link);
    if (render != NULL) {
      ...
      } else if (render->preview || render->pixels == NULL || size_changed(render)) {
        render->tex_width  = render->x2 - render->x1;
        render->tex_height = render->y2 - render->y1;
        render->preview    = FALSE;
        if (head->surface != NULL) cairo_surface_destroy(head->surface);
        head->surface           = draw_head(pango, &state->render, head->name, render->tex_width, render->tex_height);
...
static void set_clicked_head(struct wd_state *state, struct wd_render_head_data *clicked) {
  GdkFrameClock *clock = gtk_widget_get_frame_clock(state->canvas);
  uint64_t tick        = gdk_frame_clock_get_frame_time(clock);
...
static void canvas_drag1_begin(GtkGestureDrag *drag, gdouble mouse_x, gdouble mouse_y, gpointer data) {
  struct wd_state *state = data;
...
static void canvas_drag1_update(GtkGestureDrag *drag, gdouble delta_x, gdouble delta_y, gpointer data) {
  struct wd_state *state = data;

  if (state->clicked == NULL) return;
  ...
  if (dim.rotation_id & 1) SWAP(int, size.x, size.y);
  ...
  for (GList *form_iter = forms; form_iter != NULL; form_iter = form_iter->next) {
    ...
    if (other->render != state->clicked && !(mod_state & GDK_SHIFT_MASK)) {
      ...
      if (other_dim.rotation_id & 1) SWAP(int, w, h);
      double x2 = x1 + w;
      double y2 = y1 + h;
      if (fabs(br.x) <= snap) new_pos.x = -size.x;
      if (fabs(br.y) <= snap) new_pos.y = -size.y;
      if (fabs(br.x - x1) <= snap) new_pos.x = x1 - size.x;
      if (fabs(br.x - x2) <= snap) new_pos.x = x2 - size.x;
      if (fabs(br.y - y1) <= snap) new_pos.y = y1 - size.y;
      if (fabs(br.y - y2) <= snap) new_pos.y = y2 - size.y;

      if (fabs(tl.x) <= snap) new_pos.x = 0.;
      if (fabs(tl.y) <= snap) new_pos.y = 0.;
      if (fabs(tl.x - x1) <= snap) new_pos.x = x1;
      if (fabs(tl.x - x2) <= snap) new_pos.x = x2;
      if (fabs(tl.y - y1) <= snap) new_pos.y = y1;
      if (fabs(tl.y - y2) <= snap) new_pos.y = y2;
    }
  }
  wd_head_form_set_position(form, new_pos.x, new_pos.y);
}

static void canvas_drag1_end(GtkGestureDrag *drag, gdouble mouse_x, gdouble mouse_y, gpointer data) {
  struct wd_state *state = data;
  set_clicked_head(state, NULL);
  update_cursor(state);
}

static void canvas_drag2_begin(GtkGestureDrag *drag, gdouble mouse_x, gdouble mouse_y, gpointer data) {
  struct wd_state *state = data;
  state->panning         = TRUE;
...
static void canvas_drag2_update(GtkGestureDrag *drag, gdouble delta_x, gdouble delta_y, gpointer data) {
  struct wd_state *state = data;
  GtkAdjustment *xadj    = gtk_scrolled_window_get_hadjustment(GTK_SCROLLED_WINDOW(state->scroller));
  GtkAdjustment *yadj    = gtk_scrolled_window_get_vadjustment(GTK_SCROLLED_WINDOW(state->scroller));
...
static void canvas_drag2_end(GtkGestureDrag *drag, gdouble mouse_x, gdouble mouse_y, gpointer data) {
  struct wd_state *state = data;
  state->panning         = FALSE;
  update_cursor(state);
}

static void canvas_motion(GtkEventControllerMotion *controller, gdouble mouse_x, gdouble mouse_y, gpointer data) {
  struct wd_state *state = data;
  update_hovered(state, mouse_x, mouse_y);
}

static void canvas_enter(GtkEventControllerMotion *controller, gdouble x, gdouble y, gpointer data) {
  struct wd_state *state    = data;
  GdkEvent *event           = gtk_get_current_event();
  GdkModifierType mod_state = event->crossing.state;

  if (!(mod_state & GDK_BUTTON1_MASK)) set_clicked_head(state, NULL);
  if (!(mod_state & GDK_BUTTON2_MASK)) state->panning = FALSE;
  update_cursor(state);
}
...
static void canvas_leave(GtkEventControllerMotion *controller, gpointer data) {
  struct wd_state *state = data;
  struct wd_render_head_data *render;
  wl_list_for_each(render, &state->render.heads, link) { render->hovered = FALSE; }
  update_tick_callback(state);
}

static gboolean canvas_scroll(GtkEventControllerScroll *controller, gdouble delta_x, gdouble delta_y, gpointer data) {
  struct wd_state *state    = data;
  GdkEvent *event           = gtk_get_current_event();
  GdkModifierType mod_state = event->scroll.state;

  if (mod_state & GDK_CONTROL_MASK) {
    if (delta_y) zoom_to(state, state->zoom * pow(0.75, delta_y));
  } else {
    GtkAdjustment *xadj = gtk_scrolled_window_get_hadjustment(GTK_SCROLLED_WINDOW(state->scroller));
    GtkAdjustment *yadj = gtk_scrolled_window_get_vadjustment(GTK_SCROLLED_WINDOW(state->scroller));
    double xstep        = gtk_adjustment_get_step_increment(xadj);
    double ystep        = gtk_adjustment_get_step_increment(yadj);
    if (delta_x) gtk_adjustment_set_value(xadj, gtk_adjustment_get_value(xadj) + xstep * delta_x);
    if (delta_y) gtk_adjustment_set_value(yadj, gtk_adjustment_get_value(yadj) + ystep * delta_y);
  }
  return TRUE;
}

static void canvas_resize(GtkWidget *widget, GdkRectangle *allocation, gpointer data) {
  struct wd_state *state = data;
  update_scroll_size(state);
}
...
static void apply_changes(GSimpleAction *action, GVariant *param, gpointer data) { apply_state(data); }

static void info_response(GtkInfoBar *info_bar, gint response_id, gpointer data) { gtk_info_bar_set_revealed(info_bar, FALSE); }
...
static gboolean redraw_canvas(GtkWidget *widget, GdkFrameClock *frame_clock, gpointer data) {
  struct wd_state *state = data;
  if (state->capture) wd_capture_frame(state);
  update_tick_callback(state);
  queue_canvas_draw(state);
...
static void overlay_selected(GSimpleAction *action, GVariant *param, gpointer data) {
  ...
  wl_list_for_each(output, &state->outputs, link) {
    if (state->show_overlay) wd_create_overlay(output);
    else wd_destroy_overlay(output);
  }
}

static void window_state_changed(GtkWidget *window, GdkEventWindowState *event, gpointer data) {
  struct wd_state *state = data;
...
static void activate(GtkApplication *app, gpointer user_data) {
  GdkDisplay *gdk_display = gdk_display_get_default();
  if (!GDK_IS_WAYLAND_DISPLAY(gdk_display)) wd_fatal_error(1, "This program is only usable on Wayland sessions.");
  ...
  gtk_css_provider_load_from_resource(css_provider, WDISPLAYS_RESOURCE_PREFIX "/style.css");
  ...
  GtkBuilder *builder   = gtk_builder_new_from_resource(WDISPLAYS_RESOURCE_PREFIX "/wdisplays.ui");
  ...
  gtk_widget_add_events(state->canvas, GDK_POINTER_MOTION_MASK | GDK_BUTTON_PRESS_MASK | GDK_BUTTON_RELEASE_MASK
  ...
  GtkEventController *canvas_scroll_controller
      = gtk_event_controller_scroll_new(state->canvas, GTK_EVENT_CONTROLLER_SCROLL_BOTH_AXES);
  ...
  action = g_simple_action_new_stateful("auto-apply", NULL, g_variant_new_boolean(state->autoapply));
  g_signal_connect(action, "change-state", G_CALLBACK(auto_apply_selected), state);
  g_action_map_add_action(G_ACTION_MAP(main_actions), G_ACTION(action));

  GSimpleAction *capture_action = g_simple_action_new_stateful("capture-screens", NULL, g_variant_new_boolean(state->capture));
  ...
  GSimpleAction *overlay_action
      = g_simple_action_new_stateful("show-overlay", NULL, g_variant_new_boolean(state->show_overlay));
  ...
  if (state->output_manager == NULL) wd_fatal_error(1, "Compositor doesn't support wlr-output-management-unstable-v1");
  if (state->xdg_output_manager == NULL) wd_fatal_error(1, "Compositor doesn't support xdg-output-unstable-v1");
