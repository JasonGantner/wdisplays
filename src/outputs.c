...
static void config_handle_succeeded(void *data, struct zwlr_output_configuration_v1 *config) {
  ...
  if (wd_store_config(pending->outputs) == 0)
    wd_ui_show_error(pending->state, "Change was applied successfully and config was saved.");
...
static void config_handle_failed(void *data, struct zwlr_output_configuration_v1 *config) {
  ...
  wd_ui_show_error(pending->state, "The display server was not able to process your changes.");
  destroy_pending(pending);
}
...
static void config_handle_cancelled(void *data, struct zwlr_output_configuration_v1 *config) {
  ...
  zwlr_output_configuration_v1_destroy(config);
  wd_ui_apply_done(pending->state, NULL);
  wd_ui_show_error(pending->state, "The display configuration was modified by the server before updates "
                                   "were processed. "
...
void wd_apply_state(struct wd_state *state, struct wl_list *new_outputs, struct wl_display *display) {
  ...
  wl_list_for_each(output, new_outputs, link) {
    ...
    } else if (output->enabled != head->enabled || output->width != head->custom_mode.width
               || output->height != head->custom_mode.height || output->refresh != head->custom_mode.refresh) {
      zwlr_output_configuration_head_v1_set_custom_mode(config_head, output->width, output->height, output->refresh);
    }
...
static void wd_frame_destroy(struct wd_frame *frame) {
  if (frame->pixels != NULL) munmap(frame->pixels, frame->height * frame->stride);
  if (frame->buffer != NULL) wl_buffer_destroy(frame->buffer);
  if (frame->pool != NULL) wl_shm_pool_destroy(frame->pool);
  if (frame->capture_fd != -1) close(frame->capture_fd);
  if (frame->wlr_frame != NULL) zwlr_screencopy_frame_v1_destroy(frame->wlr_frame);
...
static void capture_buffer(void *data, struct zwlr_screencopy_frame_v1 *copy_frame, uint32_t format, uint32_t width,
                           uint32_t height, uint32_t stride) {
  struct wd_frame *frame = data;

  if (format != WL_SHM_FORMAT_ARGB8888 && format != WL_SHM_FORMAT_XRGB8888 && format != WL_SHM_FORMAT_ABGR8888
  ...
  if (frame->capture_fd == -1) goto err;

  frame->pool   = wl_shm_create_pool(frame->output->state->shm, frame->capture_fd, size);
...
static void capture_flags(void *data, struct zwlr_screencopy_frame_v1 *wlr_frame, uint32_t flags) {
  struct wd_frame *frame = data;
  frame->y_invert        = !!(flags & ZWLR_SCREENCOPY_FRAME_V1_FLAGS_Y_INVERT);
}

static void capture_ready(void *data, struct zwlr_screencopy_frame_v1 *wlr_frame, uint32_t tv_sec_hi, uint32_t tv_sec_lo,
                          uint32_t tv_nsec) {
  struct wd_frame *frame = data;

  frame->pixels = mmap(NULL, frame->stride * frame->height, PROT_READ, MAP_SHARED, frame->capture_fd, 0);
  ...
  wl_list_for_each_safe(frame_iter, frame_tmp, &frame->output->frames, link) {
    if (frame != frame_iter) wd_frame_destroy(frame_iter);
  }
}

static void capture_failed(void *data, struct zwlr_screencopy_frame_v1 *wlr_frame) {
  struct wd_frame *frame = data;
  wd_frame_destroy(frame);
}

struct zwlr_screencopy_frame_v1_listener capture_listener
    = {.buffer = capture_buffer, .flags = capture_flags, .ready = capture_ready, .failed = capture_failed};
...
static bool has_pending_captures(struct wd_state *state) {
  ...
  wl_list_for_each(output, &state->outputs, link) {
    ...
    wl_list_for_each(frame, &output->frames, link) {
      if (frame->pixels == NULL) return true;
    }
  }
  return false;
}
...
void wd_capture_frame(struct wd_state *state) {
  if (state->copy_manager == NULL || has_pending_captures(state) || !state->capture) return;
  ...
  wl_list_for_each(output, &state->outputs, link) {
    ...
    frame->output          = output;
    frame->capture_fd      = -1;
    frame->wlr_frame       = zwlr_screencopy_manager_v1_capture_output(state->copy_manager, 1, output->wl_output);
    zwlr_screencopy_frame_v1_add_listener(frame->wlr_frame, &capture_listener, frame);
...
static void wd_output_destroy(struct wd_output *output) {
  ...
  wl_list_for_each_safe(frame, frame_tmp, &output->frames, link) { wd_frame_destroy(frame); }
  if (output->state->layer_shell != NULL) wd_destroy_overlay(output);
...
static void wd_head_destroy(struct wd_head *head) {
  if (head->state->clicked == head->render) head->state->clicked = NULL;
  if (head->render != NULL) {
...
static void mode_handle_size(void *data, struct zwlr_output_mode_v1 *wlr_mode, int32_t width, int32_t height) {
  struct wd_mode *mode = data;
  mode->width          = width;
  mode->height         = height;
}

static void mode_handle_refresh(void *data, struct zwlr_output_mode_v1 *wlr_mode, int32_t refresh) {
  struct wd_mode *mode = data;
...
static void head_handle_name(void *data, struct zwlr_output_head_v1 *wlr_head, const char *name) {
...
static void head_handle_finished(void *data, struct zwlr_output_head_v1 *wlr_head) {
  ...
  wl_list_for_each(head, &state->heads, link) {
    if (head->id != counter) {
      head->id = counter;
      if (head->output != NULL) wd_redraw_overlay(head->output);
    }
    counter++;
  }
}
...
static void registry_handle_global(void *data, struct wl_registry *registry, uint32_t name, const char *interface,
  ...
  if (strcmp(interface, zwlr_output_manager_v1_interface.name) == 0) {
    ...
    state->copy_manager = wl_registry_bind(registry, name, &zwlr_screencopy_manager_v1_interface, 1);
...
static void output_logical_position(void *data, struct zxdg_output_v1 *zxdg_output_v1, int32_t x, int32_t y) {
...
static void output_name(void *data, struct zxdg_output_v1 *zxdg_output_v1, const char *name) {
  struct wd_output *output = data;
  if (output->name != NULL) free(output->name);
  output->name         = strdup(name);
  struct wd_head *head = wd_find_head(output->state, output);
  if (head != NULL) wd_ui_reset_head(head, WD_FIELD_NAME);
}

static const struct zxdg_output_v1_listener output_listener = {
    .logical_position = output_logical_position, .logical_size = noop, .done = noop, .name = output_name, .description = noop};
...
void wd_add_output(struct wd_state *state, struct wl_output *wl_output, struct wl_display *display) {
  ...
  output->xdg_output       = zxdg_output_manager_v1_get_xdg_output(state->xdg_output_manager, wl_output);
...
struct wd_output *wd_find_output(struct wd_state *state, struct wd_head *head) {
  ...
  if (head->output != NULL) return head->output;
  struct wd_output *output;
  wl_list_for_each(output, &state->outputs, link) {
    if (output->name != NULL && strcmp(output->name, head->name) == 0) {
      head->output = output;
...
void wd_state_destroy(struct wd_state *state) {
  ...
  wl_list_for_each_safe(head, head_tmp, &state->heads, link) { wd_head_destroy(head); }
  struct wd_output *output, *output_tmp;
  wl_list_for_each_safe(output, output_tmp, &state->outputs, link) { wd_output_destroy(output); }
  if (state->layer_shell != NULL) zwlr_layer_shell_v1_destroy(state->layer_shell);
  if (state->copy_manager != NULL) zwlr_screencopy_manager_v1_destroy(state->copy_manager);
